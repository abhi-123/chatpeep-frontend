...
const instance = axios.create({
  baseURL: `http://192.168.29.48:8082/api/v1`, // For Local Expo GO Development
...
export const saveConvoApi = async (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await instance.post("/saveconversation", data);
      resolve(response.data);
    } catch (error) {
      console.log(error?.message);
      reject(error?.message);
...
export const loadPreviousChatsByUserApi = async (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await instance.post("/loadconversation", data);
      resolve(response.data);
    } catch (error) {
      console.log(error?.message);
      reject(error?.message);
